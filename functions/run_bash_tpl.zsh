...
function run_bash_tpl() {
    ...
    if [[ -f "$input_path" ]]; then
        ...
        if [[ "$input_path" == *.template ]]; then
            local output_file=$(get_output_filename "$input_path")
            process_file "$input_path" "$output_file"
            return $?
        else
            echo "Error: File '$input_path' does not match template pattern (*.template)"
            return 1
        fi
    fi
    ...
    if [[ -d "$input_path" ]]; then
        ...
        find "$input_path" -type f -name "*.template" | while read -r input_file; do
